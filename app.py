...
def init_db():
    ...
    c = conn.cursor()
    c.execute(
        """CREATE TABLE IF NOT EXISTS chats (id TEXT PRIMARY KEY, title TEXT, messages TEXT)"""
    )
    # Thêm vào trong hàm init_db()
    c.execute("CREATE TABLE IF NOT EXISTS health_log (id TEXT, mood TEXT, note TEXT, date TEXT)")
    conn.commit()
    conn.close()
...
def save_chat_to_db(chat_id, title, messages):
    ...
    c = conn.cursor()
    c.execute(
        "INSERT OR REPLACE INTO chats (id, title, messages) VALUES (?, ?, ?)",
        (chat_id, title, json.dumps(messages)),
    )
    conn.commit()
    conn.close()
    
def load_all_chats_from_db():
    conn = sqlite3.connect(DB_FILE)
    c = conn.cursor()
        ...
        for chat_id, data in list(st.session_state.all_chats.items())[::-1]:
            ...
            if st.session_state.editing_chat_id == chat_id:
                ...
                if col_save.button("Lưu", key=f"save_{chat_id}", use_container_width=True):
                    ...
                    save_chat_to_db(chat_id, new_title, data["messages"])
                    st.session_state.editing_chat_id = None
                    st.rerun()
...
if choice == "Giải Bài Tập AI":
    ...
    # ---------------------------------------------------------
        ...
        def handle_stop():
            ...
            save_chat_to_db(curr_id, title, st.session_state.all_chats[curr_id]["messages"])
        ...
        if prompt:
            ...
            with st.chat_message("assistant", avatar="🎓"):
                ...
                if raw_response:
                    ...
                    # Lưu vào Database
                    save_chat_to_db(
                        curr_id, st.session_state.all_chats[curr_id]["title"], messages
                    )
    ...
    if final_input:
        ...
        with st.chat_message("assistant", avatar="🌿"):
            with st.spinner("EduMind đang lắng nghe..."):
                ...
                """
                res, _ = call_gemini_retry(sys_msg, "Nhanh")
            ...
            if sim_ans:
                ...
                with st.chat_message("assistant", avatar="💼"):
                    with st.spinner("Sếp đang đánh giá cách xử lý của cậu..."):
                        ...
                        """
                        res, _ = call_gemini_retry(eval_msg, "Nhanh")
                        st.markdown(res)
